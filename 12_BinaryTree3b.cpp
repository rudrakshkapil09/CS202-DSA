//setting tree using LRD and LDR
#include <iostream>
using namespace std;
...
//special functions
void revString(char list[], int n)
{
    char temp[50];
    for (int i = 0; i < n; i++)
        temp[i] = list[n-1-i];
...
void setTree(BTPTR &T, char LDR[], char LRD[], int low, int high)
{
    static int index = high;
    if (low <= high && LRD[index] != '\0') {
        int j = getRoot(LDR, LRD[index--], low, high);
        T = new(btnode);
        T->data = LDR[j];
        T->lchild = T->rchild = NULL;
        setTree(T->rchild, LDR, LRD, low, j-1);
        setTree(T->lchild, LDR, LRD, j+1, high);
    }
}
...
int main()
{
    ...
    revString(LDR, strlen(LDR));
    
    setTree(T, LDR, LRD, 0, 11);
    printTree(T);
    cout << endl;
}
