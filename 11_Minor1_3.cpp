//Minor question 3 - range of a node
#include <iostream>
using namespace std;
...
void printRange(BSTPTR T, int a[], int data)
{
    int j = 0;
    getTraversal(T, a, j);
    int i = 0;
    while (a[i] != data && i < j) i++;
    cout << "Range => " << a[i-1]+1 << " to " << a[i+1]-1 << endl;
}
...
{
    BSTPTR T = NULL;
    int list[15] = {20, 30, 40, 50, 60, 70, 80, 90}, out[20];
    setTree(T, 0, 7, list);
    insertNode(T, 65);
    insertNode(T, 10);
    cout << "Tree:\n";
