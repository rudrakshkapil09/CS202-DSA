...
{
    if (T) {
        if (T->lchild) T->lchild->dd = T->dd + 1;
        if (T->rchild) T->rchild->dd = T->dd;
        setDD(T->lchild);
        setDD(T->rchild);
    }
}
...
void printdiagonalElements(LQueue Q[])
{
    for (int i = 0; i < 10; i++) {
        if (!LqueueIsEmpty(Q[i])) {
            cout << "Level " << setw(1) << i-5 << " => ";
            while (!LqueueIsEmpty(Q[i])) {
                BTPTR T = Ldequeue(Q[i]);
                cout << T->data;
            }
            cout << endl;
        }
...
int main()
{
    ...
    T->dd = 0;
    setDD(T);
    arrangeInQueues(T, diagonal_elements);
    printTree(T);
    
    cout << "\n\ndiagonal elements:\n";
    printdiagonalElements(diagonal_elements);
